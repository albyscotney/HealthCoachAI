...
# %%
INFLUXDB_VERSION = os.getenv("INFLUXDB_VERSION",'3') # Your influxdb database version (accepted values are '1' or '3')
...
if MANUAL_START_DATE:
    ...
    try:
        if INFLUXDB_VERSION == "1":
            ...
            last_influxdb_sync_time_UTC = pytz.utc.localize(influxdbclient.query(query="SELECT * FROM HeartRateIntraday ORDER BY time ASC LIMIT 1", language="influxql").to_pylist()[0]['time'])
    except Exception as err:
        logging.error(err)
